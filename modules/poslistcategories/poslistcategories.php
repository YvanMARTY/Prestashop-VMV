...
class PosListCategories extends Module implements WidgetInterface
{
	...
	protected $default_per_sm = 4;
	protected $default_per_xs = 3;
	protected $default_per_xxs = 2;
	...
	protected function installDemoData()
	{
		...
		for ($i = 1; $i <= 6; ++$i)
		{
			...
			{
				$item->image[$language['id_lang']] = 'thumb-'.$i.'.png';
    ...
    {	
		...
		return $this->display(__FILE__, 'poslistcategories.tpl', $this->getCacheId('poslistcategories'));
    }
	...
	{
		$this->_clearCache('poslistcategories.tpl');
	}
	...
	public function getItems($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT pc.`id_poslistcategories_items` as id_item, pci.`position`, pci.`active`, pci.`id_category`, pcil.`description`, pcil.`image`
